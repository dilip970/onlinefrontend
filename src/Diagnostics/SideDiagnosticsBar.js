...
class SideDiagnosticsBar extends Component
        ...
        <div className="sidedoctor_bar">
          ...
          <Link as={Link} to="/doctors-list"><a><button className="btn theme__button">All Doctors</button></a></Link>
          {
               this.state.categories.map((categories,key) => {
                  return <a href={`/categorybasedalltest/${categories.rbcm_id}`}><button className="btn theme__button">{categories.rbcm_name}</button></a>
                })
            }
...
export default SideDiagnosticsBar;
